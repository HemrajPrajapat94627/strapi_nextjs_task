...
const TestimonialArea = () => {  
    const [homeTestimaonials, setHomeTestimaonials] = useState([]);
    
    const geHomeTestimonials = async () => {
        try {
            const response = await fetch(`${BASEURL}/api/home-testimonials?populate[avatar][populate]=true&populate[avatar][fields][0]=url`);
            const data = await response.json();
            setHomeTestimaonials(data.data);
        } catch (error) {
                        ...
                        <Swiper {...setting}>
                            {homeTestimaonials.map((item, i)  => 
                                    ...
                                    <div className="tp-testimonial-item-inner">
                                        ...
                                        <p dangerouslySetInnerHTML={{ __html: item.attributes.description }} />                                        <div className="tp-testimonial-rating d-flex justify-content-center">
                                            <span>
                                    ...
                                    <div className="tp-testimonial-user-thumb">
                                        <img src={`${BASEURL + item.attributes.avatar.data.attributes.url}`} alt="theme-pure" />
                                    </div>
                                    <div className="tp-testimonial-designation">
                                        <h4 className="testimonial-title">{item.attributes.name}</h4>

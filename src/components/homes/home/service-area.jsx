...
const ServiceArea = () => {
   ...
   const getHomeServices = async () => {
      try {
          const response = await fetch(`${BASEURL}/api/home-services2?populate[img][populate]=true&populate[img][fields][0]=url`);
                                 ...
                                 <SplideSlide key={i}>
                                       <div className="tp-service-wrapper p-relative mb-55">
                                          <div className="tp-service-designation">
                                             <p>{item.attributes.alphabet}</p>
                                          </div>
                                          <h3 className="service-title mt-3" dangerouslySetInnerHTML={{ __html: item.attributes.title }}></h3>
                                          <div className="tp-service-icon">
                                             <img src={`${BASEURL + item.attributes.img.data.attributes.url}`} alt="theme-pure" />
                                          </div>
                                          <p className="hide-text">{item.attributes.description}</p>

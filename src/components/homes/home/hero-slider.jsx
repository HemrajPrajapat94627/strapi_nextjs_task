...
const HeroSlider = () => {
  ...
  const geHomeSlider = async () => {
    try {
      const response = await fetch(`${BASEURL}/api/home-slides?populate[bg_img][populate]=true&populate[bg_img][fields][0]=url`);
      const data = await response.json();
      setHomeSlides(data.data);
    } catch (error) {
                    ...
                    <div className="container">
                      ...
                      <div className="tp-hero-1">
                        <div className="tp-hero-bg tp-hero-overlay p-relative"
                          style={{ backgroundImage: `url(${`${BASEURL + item.attributes.bg_img.data.attributes.url}`})` }}></div>
                        <div className="row">
                            ...
                            <div className="tp-hero-content p-relative">
                              <div className="tp-hero-title-wrapper">
                                <span className="tp-section-title__pre p-relative">
                                  {item.attributes.sub_title_1}{" "}
                                  <span className="title-pre-color">
                                    {item.attributes.sub_title_2}
                                  </span>
                                  <AngleArrow />
                                </span>
                                <h3 className="tp-hero-title">
                                  {item.attributes.hero_title_1} <LineArrow />
                                  <span className="title-color">
                                    {item.attributes.hero_title_2}
                                  </span>{" "}
                                  <br />{" "}
                                  <span className="title-text-transparent">
                                    {item.attributes.hero_support}
                                  </span>
                                </h3>
